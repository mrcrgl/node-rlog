...
var Format = {
  ...
  toString: function(oMessage, c) {
    var formatString = Format.getFormat(c.format || 0);

    var string = String(formatString).replace(formatArgsRegExp, function(x) {
      x = x.substr(1);
  ...
  colorize: function(string, type, c) {
    ...
    if (type === 'lvl'
      && c.lvlColors.hasOwnProperty(string)) {
      color = c.lvlColors[string];
    }

    if (!color
      && !c.colors.hasOwnProperty(type)) {
    ...
    var coloredString = dye[color](string);

...
var formats = [
  "[:date] [:app] [:severityString] [:ns] - :message",
  "[:date] [:severityString] [:ns]: :message",
  "[:date] [:severityString] :app.:ns - :message"
];
...
var Format = {
  ...
  colorize: function (string, type, c) {
    ...
    if (type === 'severity' && c.severityColors.hasOwnProperty(string)) {
      color = c.severityColors[string];
    }
